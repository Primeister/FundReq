...
app.get('/reporting/:FundManager', (req, res) => {
    ...
    // Fetch funding opportunities managed by the fund manager
    const fundingOpportunitiesQuery = `SELECT * FROM FundingOpportunity WHERE FundManager=?`;
    db.all(fundingOpportunitiesQuery, [fundManager], (err, fundingOpportunities) => {
        ...
        // Fetch all applications for these funding opportunities
        const fundingNames = fundingOpportunities.map(fo => fo.FundingName);
        const applicationsQuery = `SELECT * FROM form WHERE funding_name IN (${fundingNames.map(() => '?').join(',')})`;
        db.all(applicationsQuery, fundingNames, (err, applications) => {
            if (err) {
            ...
            const fundingOpportunityStatusData = fundingOpportunities.map(fo => {
                const foApplications = applications.filter(app => app.funding_name === fo.FundingName);
                return {
                    fundingName: fo.FundingName,
                    processing: foApplications.filter(app => app.status === 'processing').length,
                    approved: foApplications.filter(app => app.status === 'Approved').length,
                    rejected: foApplications.filter(app => app.status === 'Rejected').length
                };
            });
            ...
            // Create the charts using Chart.js and canvas
            const canvas = createCanvas(800, 600);
            const ctx = canvas.getContext('2d');

            // Example of creating a bar chart for funding opportunities
            const chart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: fundingOpportunityStatusData.map(data => data.fundingName),
                    datasets: [
                        {
                            label: 'Processing',
                            data: fundingOpportunityStatusData.map(data => data.processing),
                        ...
                        {
                            label: 'Approved',
                            data: fundingOpportunityStatusData.map(data => data.approved),
                        ...
                        {
                            label: 'Rejected',
                            data: fundingOpportunityStatusData.map(data => data.rejected),
